...
(async function() {
    ...
    async function startMainScript(gameCanvas) {
        ...
        const POPUP_CLOSE_POINT = {x: 0.971527, y: 0.050058}
        ...
        async function closeHomePopups(macro) {
            ...
            while (Date.now() - started < HOME_POPUP_WATCH_MS && isRunningMacro == macro) {
                if (await screenMatches(screenHome, 20)) {
                    if (clicks > 0) diagLog('popup', 'домашний экран свободен, кликов по крестику: ' + clicks)
                    return true
                }

                clicks++
                if (clicks === 1) diagLog('popup', 'домашний экран закрыт попапом, кликаем по крестику')
                await runActions(
                    [{
                        x: POPUP_CLOSE_POINT.x, y: POPUP_CLOSE_POINT.y,
                        actionType: actionClick, title: 'Closing popup',
                        delay: HOME_POPUP_CLICK_EVERY_MS, skipLog: true
                    }],
                    macro, 0
                )
            }

            diagLog('popup', 'домашний экран так и не появился за ' + Math.round(HOME_POPUP_WATCH_MS / 1000) + ' с, кликов: ' + clicks)
            return false
        }
